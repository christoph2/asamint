...
def test_run_uses_creator_and_daq_capture(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class StubDaqList:
        def __init__(self, name: str, event_num: int) -> None:
            self.name = name
            self.event_num = event_num
            self.stim = False
            self.enable_timestamps = True
            self.measurements = [("sig1", 0, 0, "U8")]
    ...
    assert captured["capture_kwargs"]["daq_lists"][0].name == "daq_list"
    metadata = captured["capture_kwargs"]["metadata"]
    assert metadata["daq_timebase_hint_s"] == pytest.approx(0.01)
    assert metadata["daq_list_names"] == ["daq_list"]
    assert result.timebases is not None
    assert result.timebases[0]["timebase_s"] == pytest.approx(0.01)
    assert result.timebases[0]["group_id"] == 1
