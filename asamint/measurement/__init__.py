...
def _persist_csv_format(
    ...
    **_: Any,
    ...
    target = (
        ...
        else _auto_filename("measurement", ".csv")
    )
    return finalize_measurement_outputs(
...
def _persist_hdf5_format(
    ...
    **_: Any,
    ...
    target = (
        str(output_path)
        if output_path is not None
        else _auto_filename("measurement", ".h5")
    )
    return finalize_measurement_outputs(
        data=data,
        units=units,
...
def _run_daq_flow(
    ...
    *,
    ...
    daq_lists = (
        daq_lists
        if daq_lists is not None
        else build_daq_lists(creator.session, daq_groups)
    )
...
def _run_daq_flow_wrapper(
    ...
    if not fmt.supports_live_capture or fmt.creator_factory is None:
        raise ValueError(
            f"Measurement format '{format_name}' does not support DAQ capture."
        )
    ...
    try:
        ...
        if result.hdf5_path:
            ...
            _annotate_daq_hdf5_metadata(
                ...
                project_meta,
                timebase_hint_s=(
                    result.timebases[0]["timebase_s"] if result.timebases else None
                ),
            )
        return result
    finally:
