...
class SwFeatureClassInstances(Base):
    ...
    t = StdString()
    si = StdString()
    sw_feature_export_class_instances_id = Column(
        types.Integer,
        ForeignKey("sw_feature_export_class_instances.rid", use_alter=True),
    )
    sw_feature_export_class_instances = relationship(
        "SwFeatureExportClassInstances",
        foreign_keys=[sw_feature_export_class_instances_id],
        uselist=False,
        cascade="all",
    )
    sw_feature_import_class_instances_id = Column(
        types.Integer,
        ForeignKey("sw_feature_import_class_instances.rid", use_alter=True),
    )
    sw_feature_import_class_instances = relationship(
        "SwFeatureImportClassInstances",
        foreign_keys=[sw_feature_import_class_instances_id],
        uselist=False,
        cascade="all",
    )
    sw_feature_local_class_instances_id = Column(
        types.Integer,
        ForeignKey("sw_feature_local_class_instances.rid", use_alter=True),
    )
    sw_feature_local_class_instances = relationship(
        "SwFeatureLocalClassInstances",
        foreign_keys=[sw_feature_local_class_instances_id],
        uselist=False,
        cascade="all",
    )
...
class SwVcdCriterion(Base):
    ...
    sw_vcd_criterion_possible_values = relationship(
        "SwVcdCriterionPossibleValues",
        foreign_keys=[sw_vcd_criterion_possible_values_id],
        uselist=False,
        cascade="all",
    )
    sw_compu_method_ref_id = Column(
